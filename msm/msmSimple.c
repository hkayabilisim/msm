...
int Mx, My, Mz;           // # grid points and range
...
double gLRprime(double r) {
  if (r < DBL_EPSILON)
    ...
    return (2.0 * beta * exp(-beta * beta * r * r)) / (sqrt(MYPI) * r)
        - erf(beta * r) / (r * r);
}
...
double g1prime(double r) {
  return (1.0 / (a * a)) * gamaprime(r / a) - gLRprime(r);
}
...
void calculate_ulong_self() {
  ...
  for (int i = 0; i < N; i++)
    q2sum += q[i] * q[i];
  ulong_self = - 0.5 * q2sum * (1.0 / a) * gama(0);
}
...
void calculate_ulong_real() {
  ...
  for (int mx = 0; mx < (Mxmax-Mxmin+1); mx++) {
    for (int my = 0; my < (Mymax-Mymin+1); my++) {
      for (int mz = 0; mz < (Mzmax-Mzmin+1); mz++) {
      ...
      for (int kx = -kmax; kx <= kmax; kx++) {
        for (int ky = -kmax; ky <= kmax; ky++) {
          for (int kz = -kmax; kz <= kmax; kz++) {
            if (kx == 0 && ky == 0 && kz == 0)
              continue;
            double k2 = (kx / Ax) * (kx / Ax) + (ky / Ay) * (ky / Ay)
                + (kz / Az) * (kz / Az);
...
void display_results() {
...

  {
    double ftotal_max = -1;
    for (int i = 0; i < N; i++) {
      double relerror = diffnorm(ftotal[i], ftotal_expected[i], 3) / norm(ftotal_expected[i],3);
      if (relerror > ftotal_max)
        ftotal_max = relerror;
    }
  ...
  {
    FILE *fp = fopen("simple.acc", "w");
    fprintf(fp, "%d\n", N);
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][X]);
      fprintf(fp, "%25.16e ", flong_real[i][X]);
      fprintf(fp, "%25.16e ", flong_four[i][X]);
      fprintf(fp, "%25.16e ", ftotal[i][X]);
      fprintf(fp, "\n");
    }
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][Y]);
      fprintf(fp, "%25.16e ", flong_real[i][Y]);
      fprintf(fp, "%25.16e ", flong_four[i][Y]);
      fprintf(fp, "%25.16e ", ftotal[i][Y]);
      fprintf(fp, "\n");
    }
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][Z]);
      fprintf(fp, "%25.16e ", flong_real[i][Z]);
      fprintf(fp, "%25.16e ", flong_four[i][Z]);
      fprintf(fp, "%25.16e ", ftotal[i][Z]);
      fprintf(fp, "\n");
    }
  }
  {
    FILE *fp = fopen("simple.acc_expected", "w");
    fprintf(fp, "%d\n", N);
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][X]);
      fprintf(fp, "%25.16e ", flong_real_expected[i][X]);
      fprintf(fp, "%25.16e ", flong_four_expected[i][X]);
      fprintf(fp, "%25.16e ", ftotal_expected[i][X]);
      fprintf(fp, "\n");
    }
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][Y]);
      fprintf(fp, "%25.16e ", flong_real_expected[i][Y]);
      fprintf(fp, "%25.16e ", flong_four_expected[i][Y]);
      fprintf(fp, "%25.16e ", ftotal_expected[i][Y]);
      fprintf(fp, "\n");
    }
    for (int i = 0; i < N; i++) {
      fprintf(fp, "%25.16e ", fshort[i][Z]);
      fprintf(fp, "%25.16e ", flong_real_expected[i][Z]);
      fprintf(fp, "%25.16e ", flong_four_expected[i][Z]);
...
double Bspline(int k, double u) {
  if (k == 1) {
    ...
    return u / (double) (k - 1) * Bspline(k - 1, u)
        + (k - u) / (double) (k - 1) * Bspline(k - 1, u - 1.0);
  }
}
